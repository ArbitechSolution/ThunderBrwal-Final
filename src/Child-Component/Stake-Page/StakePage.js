      ...
      try {
        ...
        if (tamountlp > 0) {
          if (timestamp >= AddTime) {
            ...
            toast.error(`Unlock Time Not Reached!   Remaining Time: ${userDepositTime.days} Days ${userDepositTime.hours} hours ${userDepositTime.minutes} Minutes ${userDepositTime.seconds} seconds`)
          }
        ...
        <div className='row d-flex justify-content-center align-items-center pb-3'>
          <div className='col-md-12 col-11 Stakeboxs pt-4 pb-4'>
            <div className='row '>
              ...
              <button onClick={() => getAccount()} className='btn btnstake'>{acc === "No Wallet" ? "Connect " : acc === "Connect Wallet" ? "Connect" : acc === "Connect to Rinkebey" ? "Connect" : acc.substring(0, 5) + "..." + acc.substring(acc.length - 5)}</button>
